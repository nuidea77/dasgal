...
const base: UserProfile = { name: 'Bat', age: 28, sex: 'male', heightCm: 175, weightKg: 72, goal: 'tone', level: 'beginner', programDays: 14, daysPerWeek: 4 };

describe('plan generator', () => {
  it('produces the requested number of days and clamps to 7..30', () => {
    ...
    expect(generatePlan({ ...base, programDays: 99 }, { startDate: '2026-01-01' }).days).toHaveLength(30);
  });
  ...
  it('avoids high-impact moves for obese users', () => {
    const plan = generatePlan({ ...base, weightKg: 120, level: 'advanced', programDays: 30 }, { startDate: '2026-01-01', seed: 3 });
  ...
  it('progresses volume week over week', () => {
    const plan = generatePlan({ ...base, programDays: 21 }, { startDate: '2026-01-01', seed: 5 });
